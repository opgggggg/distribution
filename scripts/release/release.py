...
def notes_file(repo, v):
    """The release input: one notes file per version, named for that version.

    Internal release reports (`1.4.5-huawei.md`) live beside it and are
    deliberately not named `v<version>.md`: they record how a release went,
    not what it says to its users.
    """
    return Path(repo) / 'releases' / ('v' + v + '.md')


def prepared_version(repo):
    """The release being asked for: the highest version with a notes file."""
    found = []
    for p in (Path(repo) / 'releases').glob('v*.md'):
        name = p.name[1:-3]
        if re.fullmatch(r'(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)', name):
            found.append(name)
    require(found, 'No release is prepared; write releases/v<version>.md first')
    return max(found, key=version)


def android_code(v):
    """1.4.6 -> 1004006, the layout every published CubeOffice build uses."""
    major, minor, patch = version(v)
    require(minor < 1000 and patch < 1000, 'Version does not fit the Android versionCode layout')
...
def main():
    ...
    if cmd == 'get':
    ...
    elif cmd == 'input-version':
        print(prepared_version(args[0]))
    elif cmd == 'input-code':
        print(android_code(args[0]))
    elif cmd == 'input-notes':
        path = notes_file(args[0], args[1])
        require(path.is_file(), 'No release notes: write ' + str(path) + ' first')
        notes_text(path)
        print(path)
    elif cmd == 'check-new':
        v, code = args
        ...
        s = {'version': v, 'androidVersionCode': int(code), 'distribution': output('git', '-C', src, 'rev-parse', 'HEAD'),
             'upstream': output('git', '-C', src / 'als-office', 'rev-parse', 'HEAD'), 'notes': notes_text(notes_file(src, v))}
