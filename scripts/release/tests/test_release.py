...
class ReleaseInput(unittest.TestCase):
    ...
    def prepared(self, *names):
        ...
        (root / 'releases').mkdir()
        for name in names:
            (root / 'releases' / name).write_text('改进编辑。 Improved editing.')
        return root

    def test_highest_prepared_version_wins(self):
        root = self.prepared('v1.4.5.md', 'v1.4.6.md', 'v1.4.10.md')
        self.assertEqual(release.prepared_version(root), '1.4.10')

    def test_internal_reports_are_not_release_input(self):
        root = self.prepared('v1.4.6.md', '1.4.5-huawei.md', '1.4.2-harmony.md', 'v1.4.7-rc1.md')
        self.assertEqual(release.prepared_version(root), '1.4.6')

    def test_no_notes_file_is_a_clear_refusal(self):
        root = self.prepared()
        with self.assertRaisesRegex(ValueError, 'releases/v<version>.md'):
            release.prepared_version(root)

    def test_android_code_follows_the_published_layout(self):
        self.assertEqual(release.android_code('1.4.6'), 1004006)
        self.assertEqual(release.android_code('1.4.5'), 1004005)
        with self.assertRaises(ValueError):
            release.android_code('1.1000.0')

    def test_prepare_keeps_the_notes_under_the_version_name(self):
        root = self.prepared()
        (root / 'profiles/cubeoffice/desktop').mkdir(parents=True)
        (root / 'profiles/cubeoffice/desktop/catalog.mjs').write_text('version: "1.4.5"')
        release.write(root / 'profiles/cubeoffice/app.json', {'versionName': '1.4.5', 'versionCode': 1004005})
        (root / 'website').mkdir()
        for page in release.PAGES:
            (root / 'website' / page).write_text('CubeOffice 1.4.5 /downloads/CubeOffice-1.4.5-Android-b1004005.apk')
        notes = root / 'releases/v1.4.6.md'
        notes.write_text('# CubeOffice 1.4.6\n\n修复若干问题。\n\nFixed several issues.\n')
        subprocess.run(['python3', str(HELPERS / 'release.py'), 'prepare', str(root), '1.4.6', '1004006', str(notes)], check=True)
        self.assertIn('Fixed several issues.', (root / 'releases/v1.4.6.md').read_text())
        self.assertFalse((root / 'releases/1.4.6.md').exists())

    def helper(self, *args):
        return subprocess.run(['python3', str(HELPERS / 'release.py'), *map(str, args)], capture_output=True, text=True)

    def test_the_shell_reads_version_code_and_notes_from_the_file(self):
        root = self.prepared('v1.4.6.md')
        self.assertEqual(self.helper('input-version', root).stdout.strip(), '1.4.6')
        self.assertEqual(self.helper('input-code', '1.4.6').stdout.strip(), '1004006')
        self.assertEqual(self.helper('input-notes', root, '1.4.6').stdout.strip(), str(root / 'releases/v1.4.6.md'))

    def test_a_version_without_notes_stops_the_release(self):
        root = self.prepared('v1.4.6.md')
        result = self.helper('input-notes', root, '1.4.7')
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('v1.4.7.md', result.stderr)

    def test_notes_must_carry_both_languages(self):
        root = self.prepared()
        (root / 'releases/v1.4.6.md').write_text('只有中文。\n')
