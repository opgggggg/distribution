...
usage() {
  cat <<'EOF'
Usage: bash scripts/release-cubeoffice.sh [COMMAND] [options]

Prepare a release by writing releases/v<version>.md — the version, the Android
build number and the bilingual notes are read from it, so the usual release is:
...
# A release is asked for by writing releases/v<version>.md. The version, the
# Android build number and the notes all come from that one file, so nothing
# about a release is typed twice; the flags below remain for the exceptions.
[ -n "$VERSION" ] || VERSION="$(python3 "$HELPERS/release.py" input-version "$REPO")" || die 'Write releases/v<version>.md first'
[[ "$VERSION" =~ ^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$ ]] || die 'Use --version X.Y.Z'
[ -n "$ANDROID_CODE" ] || ANDROID_CODE="$(python3 "$HELPERS/release.py" input-code "$VERSION")" || die "Cannot derive an Android versionCode for $VERSION"
[ -n "$NOTES" ] || NOTES="$(python3 "$HELPERS/release.py" input-notes "$REPO" "$VERSION")" || die "Write $REPO/releases/v$VERSION.md first"
