...
async function validatePrefix(label, file, magic) {
  ...
  if (!prefix.equals(magic))
    throw new Error(`${label} has unexpected file magic: ${file}`);
}
...
async function main() {
  ...
  await copy(
    signatures.mac.signaturePath,
    path.join(updates, `${names.macUpdater}.sig`),
  );
  await copy(inputs.windows, path.join(updates, names.windows));
  await copy(
    signatures.windows.signaturePath,
    path.join(updates, `${names.windows}.sig`),
  );
  await copy(inputs.linuxAppImage, path.join(updates, names.linuxAppImage));
  await copy(
    signatures.linux.signaturePath,
    path.join(updates, `${names.linuxAppImage}.sig`),
  );
  ...
  await writeFile(
    path.join(downloads, "SHA256SUMS.txt"),
    `${checksums.join("\n")}\n`,
  );
