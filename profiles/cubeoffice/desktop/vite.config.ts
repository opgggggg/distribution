...
export default defineConfig(async (environment) => mergeConfig(
	...
	{
		plugins: [cubeOfficeLargeTextPlugin(), cubeOfficeTextContributionPlugin()],
		resolve: { alias: [{ find: /^@yaochn\/als-office-editor-ui\/document-preview$/, replacement: fileURLToPath(new URL("./document-preview.ts", import.meta.url)) }] },
	},
));
