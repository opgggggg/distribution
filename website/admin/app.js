...
function renderOverview(payload) {
	...
	for (const item of payload.versions || []) {
		...
		row.append(element("span", "", item.version || "未知"), element("strong", "", item.clients));
...
function showDetail(item) {
	...
	const blocks = [
		...
		detailBlock("版本 / 平台", `${item.app_version || "—"} · ${item.platform || "—"} · ${item.arch || "—"}`),
	...
	if (item.context) blocks.push(detailBlock("Context", JSON.stringify(item.context, null, 2), true));
	if (item.system_info)
