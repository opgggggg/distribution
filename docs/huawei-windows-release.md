...
2026-09-16 首次上架 Windows 应用时实际踩到的规格，比文档页上的提示更严格：

| 项            | 实测要求                                                                                                                         |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| 应用分类      | 创建对话框只有「应用 / 游戏」两项；二级分类在应用信息页选，CubeOffice 选「应用 / 商务」                                          |
| DisplayName   | 不在创建对话框里，是**上传软件包时**填的字段                                                                                     |
| 应用图标      | 216×216 PNG，≤500KB，必须与包内图标一致                                                                                          |
| 应用介绍截图  | 3~5 张，**必须正好 1280×720**：页面提示写「最低尺寸 1280\*720px、16:9」，但 1600×900 和 1920×1080 都被「图片尺寸不符合要求」拒掉 |
| 新版本特性    | 不能用「- 」开头的列表加换行，否则校验报「请输入新版本特性」；要写成一段连续文字                                                 |
